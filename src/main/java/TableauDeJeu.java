public class TableauDeJeu {
        ...
        for (int i = 0; i < tableau.length; i++) {
            for (int j = 0; j < tableau[i].length; j++) {
                if (tableau[i][j].getValeur() > 0) {
                    if (i == 0 && j == 0) {
                        if (
                            tableau[i][j].getValeur() !=
                                tableau[i + 1][j].getValeur() &&
                            tableau[i][j].getValeur() !=
                            tableau[i][j + 1].getValeur()
                        ) {
                            compteur++;
                        }
                    } else if (i == 0 && j == 3) {
    ...
    public void mouvementVertical(int ligne, int colonne, String direction) {
        ...
        if (
            initiale.getValeur() == 0 ||
            initiale.getValeur() == comparer.getValeur()
        ) {
            if (
                ligne > bordure ||
                (direction.equals("descendre") && (ligne < bordure))
            ) {
    ...
    private void mouvementHorizontale(
        int ligne,
        int colonne,
        String direction
    ) {
        ...
        if (
            initial.getValeur() == 0 ||
            initial.getValeur() == compare.getValeur()
        ) {
            if (
                colonne > bordure ||
                (direction.equals("droite") && (colonne < bordure))
            ) {
